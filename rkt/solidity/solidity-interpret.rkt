  ...
  (class simulator-rosette%
    ...
    (define (attack-synthesis cfg-file-loc state prog-size)
      ...
      (for ([i (range 1 prog-size)] #:break has-sol)
        (let* ([sketch (method-list i thread-size)] [binding (solve (assert (query-model sketch)))])
              (begin
                  (if (sat? binding)
    ...
    (define/public (init-analyzer cfg-file-loc parser-arg printer-arg cfg-json state [ref #f])
      ...
      (set! id->addr (map (lambda (x y) (cons (bv x BV8?) (hash-ref y `address))) 
                          (range (length blocks-raw)) blocks-raw))

      (set! addr->id (map (lambda (x y) (cons (hash-ref y `address) (bv x BV8?))) 
                          (range (length blocks-raw)) blocks-raw))
      ...
      (for ([blk-json blocks-raw])
        (begin
          (let ([cur-blk (basic-block (get-item addr->id (hash-ref blk-json `address))
                       ...
                       (map (lambda (x) (get-item addr->id x)) (hash-ref blk-json `succs)))])
      ...
      (set! method-list (foldl (lambda (x result)
