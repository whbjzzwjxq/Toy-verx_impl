...
(require  
  "solidity/solidity-parser.rkt"
  "solidity/solidity-printer.rkt"
  "solidity/solidity-machine.rkt"
  "solidity/solidity-interpret.rkt"
  json racket/sandbox
)
...
(current-bitwidth #f)
(define cfg-file-loc "./contract/escrow.bytecode")
(define cmd (string-append "python3 smart_opt.py " cfg-file-loc))

(define cfg-str
  (with-handlers ([(lambda (v) #t) (lambda (v) 'timeout)])
    (call-with-limits vandal-timeout #f (lambda () (with-output-to-string (lambda() (system cmd)))))))
